...
export async function GET(request: NextRequest) {
  try {
    ...
    // DEBUG: First check ALL profiles to see their is_admin status
    console.log("[ADMIN API] DEBUG: Fetching ALL profiles to check is_admin status...");
    ...
    if (allError) {
      console.error("[ADMIN API] DEBUG Error fetching all profiles:", allError);
    } else {
      console.log("[ADMIN API] DEBUG: Total profiles in database:", allProfiles?.length);
      console.log("[ADMIN API] DEBUG: All profiles:", allProfiles?.map(p => ({ email: p.email, is_admin: p.is_admin })));
    }

    // Now get only those with is_admin = true
    console.log("[ADMIN API] Query is_admin=true");
    ...
    console.log("[ADMIN API] Query is_admin=true returned:", adminProfiles?.length, "rows");
    console.log("[ADMIN API] Admin profiles:", adminProfiles?.map(p => ({ email: p.email, is_admin: p.is_admin })));
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingProfile) {
      ...
      if (verifyError) {
        console.error("[ADMIN API POST] Verification read error:", verifyError);
      } else {
        console.log("[ADMIN API POST] Verification result - profile is_admin status:", verifyProfile?.is_admin, "for email:", verifyProfile?.email);
      }
